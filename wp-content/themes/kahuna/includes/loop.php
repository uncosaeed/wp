...
function kahuna_continue_reading_link() {
	...
	return '<a class="continue-reading-link" href="'. esc_url( get_permalink() ) . '"><span>' . wp_kses_post( $kahuna_excerptcont ). '<i class="icon-continue-reading"></i></span><em class="screen-reader-text">"' . get_the_title() . '"</em></a>';
}
...
function kahuna_posted_edit() {
	edit_post_link( sprintf( __( 'Edit %s', 'kahuna' ), '<em class="screen-reader-text">"' . get_the_title() . '"</em>' ), '<span class="edit-link icon-metas"><i class="icon-edit icon-metas"></i> ', '</span>' );
}; // kahuna_posted_edit()
endif;
	...
	if ( ! empty( $featured_image[0] ) ) {
		...
		<div class="post-thumbnail-container" <?php cryout_schema_microdata( 'image' ); ?>>
			...
			<picture class="responsive-featured-image">
				...
				<?php if ( cryout_on_landingpage() ) { ?><source sizes="<?php echo cryout_gen_featured_sizes( $featured_width, $options['kahuna_magazinelayout'], $options['kahuna_landingpage'] ) ?>" srcset="<?php echo cryout_get_picture_src( $fimage_id, 'kahuna-featured-lp' ); ?> <?php printf( '%sw', $featured_width ) ?>">
				<?php } ?>
